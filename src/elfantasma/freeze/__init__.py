...
def freeze(atoms, x0, x1):
    ...
    print("Filled grid with atom positions:")
    print("    x0: %g" % x0[0])
    print("    y0: %g" % x0[1])
    print("    z0: %g" % x0[2])
    print("    x1: %g" % x1[0])
    print("    y1: %g" % x1[1])
    print("    z1: %g" % x1[2])
    print("    num elements: %d" % grid.size)
    print("    num filled: %d" % numpy.sum(grid))
    ...
    print("    Total volume %g m^3" % (total_volume * 1e-10 ** 3))
    print("    Sample volume %g m^3" % (filled_volume * 1e-10 ** 3))
    print("    Volume of water: %g m^3" % (remaining_volume * 1e-10 ** 3))
    print("    Density of water: %g kg/m^3" % density_of_water)
    print("    Mass of water: %g kg" % (mass_of_water / 1000))
