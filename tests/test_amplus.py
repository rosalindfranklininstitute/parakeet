...
def test_simulate_exit_wave(config_path):
    ...
    amplus.command_line.simulate.exit_wave(["-c", config, "-s", sample, "-e", exit_wave])
...
def test_analyse_average_particles(config_path):
    ...
    amplus.command_line.analyse.average_particles(["-c", config, "-s", sample, "-r", rec, "-h1", half1, "-h2", half2])
