...
def test_dose_symmetric():
    scan = parakeet.scan.new(
        mode="dose_symmetric", axis=(1, 2, 3), start_angle=-8.5, num_images=8, step_angle=2
    )
    axis = np.array((1, 2, 3)) / np.linalg.norm((1, 2, 3))
    assert np.allclose(scan.axes, np.array([axis]*8))
    assert np.allclose(scan.angles, np.array([-0.5, 1.5, -2.5, 3.5, -4.5, 5.5, -6.5, -8.5]))
    assert np.allclose(scan.position, np.array([(0, 0, 0) for i in range(8)]))
...
def test_nhelix():
    ...
    positions = np.concatenate([
            [(0, 0, 0) for i in range(0, 80, 10)],
            [(0, 10, 0) for i in range(0, 80, 10)]])
